...
class ToolsManagerRoute(View):
    template_name = 'ToolsManagerRoute.html'
    ...
    def post(self, request):
        if request.user.is_staff:
            if request.FILES['myfile']:
                ...
                # PRODUCTUIN ROUTE
                return redirect(
                    f"/media/OhhRazom/Excel/ManagerToolsRoutes/change_route_{datetime.datetime.now().strftime('%d_%m_%Y')}.xlsx")
